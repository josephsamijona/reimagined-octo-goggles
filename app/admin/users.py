...
class InterpreterAdmin(admin.ModelAdmin):
    ...
    def send_contract_invitation(self, request, queryset):
        ...
        for interpreter in queryset:
            ...
            # Send email (Placeholder - actual call to be implemented in Phase 5)
            # send_contract_invitation_email(invitation)
            
            sent_count += 1
        
        msg = f"Contract invitation sent to {sent_count} interpreter(s)."
        if skipped_count:
            msg += f" Skipped {skipped_count} interpreters who already have active invitations."
